---
log:
2026-09-05: Fixed the atomic session-store write, which did not work on Windows at all. `_write_data` renames a temp file over `self.path` while `_lock_exclusive` still holds an open handle on that same path -- the handle the caller reads through. POSIX permits a rename over an open file, so the change shipped green; Windows refuses it unless every handle was opened with `FILE_SHARE_DELETE`, which `open()` does not request, and `os.replace` raised `PermissionError: [WinError 5]`. The blast radius was total rather than marginal: `sessions.json` could not be written by any code path, so every `colab new` failed to record its session and the runtime became the orphaned, quota-holding VM that this very commit (8b518e1) had been written to prevent -- reached by a different road. Reproduced directly through `StateStore.add()`, not only in tests. The fix closes `f` immediately before `os.replace`; that is safe because the handle exists only to be read (the lock has lived in a separate `.lock` file since the move off `fcntl`, so the handle carries no locking duty), every caller has finished reading before it writes, and the enclosing `with open(...)` closing an already-closed file is a no-op. The exclusive lock is held across the whole operation, so nothing can interleave. Measured on Windows: `tests/test_state.py` went from 11 failed / 6 passed to 17 passed, and the full suite from 34 failed / 318 passed to 23 failed / 339 passed (23/339 in three of four consecutive runs; the fourth, on a loaded machine, took 68s instead of ~25s and lost `test_readers_are_concurrent_across_processes` and `test_state_store_multiprocess_concurrency`, which are wall-clock-sensitive cross-process tests -- `test_state.py` alone passed 17/17 five times running, so treat those two as load-flaky rather than as part of the baseline). Added `test_store_write_closes_handle_before_replace` in `tests/test_windows_compat.py`, which asserts on the handle's `closed` flag at the moment `os.replace` is called rather than on the platform, so a regression fails on Linux CI instead of waiting for a Windows user; verified non-vacuous by removing the `f.close()` and watching it fail. Also taught that file's `open()`-without-`encoding=` guard to skip comments (via `tokenize`, not a cut at the first `#`, which would corrupt a line holding a `#` inside a string): it is a line regex and flagged the prose *explaining* this fix as an offender. Non-vacuity re-verified by planting a real `open(self.path, "r")` in `state.py` and confirming it is still reported, with the original line and correct line number.
2026-09-05: Routed every per-account file through `colab_cli.paths.config_home()`, overridable with `COLAB_CLI_HOME`. `TOKEN_CONFIG_PATH` was a module-level `os.path.expanduser("~/.config/colab-cli/token.json")` while `--config` relocated `sessions.json` alone, so there was no supported way to hold two Google accounts at once -- and the half-measure was actively harmful rather than merely missing: pointing `--config` at a second account's store loaded THAT account's session names under the FIRST account's cached credentials, so every name referred to a runtime those credentials could not reach, and the resulting errors surfaced at `status`/`exec` time rather than at the point of the mistake. Found while building a wrapper that switches accounts when one hits its quota. The helper is its own module, importing nothing but `os`, so `auth`, `state`, `history` and `common` can all reach it without importing each other -- putting it in `auth` would have made `history` pull in google-auth to build a path. The first cut moved the token, the sessions and the settings and left `history/` and the `enable_update_check` hint behind; the history is keyed by session name and session names collide across accounts, so a shared directory interleaves two accounts' events inside one file that reads as a single timeline, and the hint named a `settings.json` the CLI would never read. `tests/test_config_home.py` covers the default, the override, tilde expansion, token and history placement, and scans the package for any remaining `~/.config/colab-cli` literal -- verified non-vacuous by restoring the literal in `history.py` and watching two tests fail. Read once at import, so the variable must be set before the process starts; that suits the wrapper case (child env) and keeps `TOKEN_CONFIG_PATH` a constant the existing tests import.
2026-09-04: Made `_LockedFileStore._write_data` atomic and added `colab stop --endpoint`. The store rewrote `sessions.json` in place (`seek(0); truncate(); write()`), leaving a window in which the file is EMPTY; a process killed in that window -- SIGKILL, a killed wrapper, power loss -- erased every session name on disk while the runtimes kept running on the server. The consequence is worse than a lost cache: an assignment lives on the server but its NAME lives only in this file, so the runtime became unreachable -- `colab sessions` printed it as `[?]`, `colab stop` had no name to take, and `colab new` then failed with "Allocation refused (precondition failed)" because the orphan still held the quota. Reached by killing a polling wrapper mid-write, which left the store as `{}` and stranded a live T4 with no supported way to release it. Writes now go to a sibling temp file, are fsynced, and are `os.replace`d over the target (atomic on POSIX and Windows), so a reader sees the old content or the new, never nothing; the caller keeps its exclusive lock and still owns `f`, which is left untouched. For runtimes already orphaned, `colab stop --endpoint <endpoint>` terminates by the endpoint `colab sessions` already prints -- `client.unassign` needs only the endpoint plus the client's own credentials, never the per-session token that a lost store takes with it. `--session` and `--endpoint` together exit 2 rather than silently preferring one; a locally-named endpoint still has its store entry removed so the two paths cannot diverge from the server; the history event records `by: endpoint` so the recovery path stays distinguishable from a normal stop. Also dropped the UTF-8 BOM `state.py` picked up in 11c0f6a -- CPython tolerates it, but it fails a plain `ast.parse` of the decoded text and shows as a spurious first-line diff.
...
- **Token Storage**: Credentials are persisted to `<config_home>/token.json` after the initial flow, where `<config_home>` is `COLAB_CLI_HOME` if set and `~/.config/colab-cli` otherwise. Every other per-account file -- `sessions.json`, `settings.json`, `colab.log`, `history/` -- resolves through the same helper (`colab_cli.paths.config_home`), so switching that one variable switches the account wholesale. Moving the token without the sessions, or the reverse, authenticates one account against another's session names.
