---
log:
2026-09-04: Forced `sys.stdout`/`sys.stderr` to UTF-8 (`errors="replace"`) in `cli.main()` before Typer dispatches. `display_output` writes kernel stream output straight to `sys.stdout`, and a Windows console defaults to the ANSI codepage (cp1252 on a Western install), so any non-ASCII character a remote script logs raised `UnicodeEncodeError` and killed the command *after* the VM had already finished the work -- the result was lost to the printer, not to the job. Hit live while polling a long-running T4 job whose script logged Vietnamese progress lines: every `colab exec` died on `ấ`, making a healthy run unobservable. This is the output-side twin of the `encoding="utf-8"` entry below, which fixed the input side. Fixed at the entry point rather than per write site because `execution.py` alone has three (`stream.write` plus two `sys.stderr.write` traceback paths) and any new one would reintroduce it. Guarded with `hasattr(stream, "reconfigure")`: pytest capture objects and some IDE consoles substitute streams that lack it, and those are not the console codepage that causes this.
